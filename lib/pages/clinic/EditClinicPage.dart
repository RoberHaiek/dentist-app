...
class _EditClinicPageState extends State<EditClinicPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _save() async {
    ...
    try{
      ...
      if(mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(localization.get('profile_saved')),backgroundColor:const Color(0xFF7DD3C0)));
    }catch(e){
      if(mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('${localization.get('save_failed')}: $e'),backgroundColor:Colors.red));
      ...
      body: NestedScrollView(
        headerSliverBuilder:(ctx,_)=>[
          SliverAppBar(
            ...
            actions:[
              TextButton.icon(
                onPressed:()=>ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(localization.get('patient_view_coming_soon')))),
                icon:const Icon(Icons.visibility_outlined,color:Colors.white,size:18),
                label:Text(localization.get('view_as_patient'),style:const TextStyle(color:Colors.white,fontSize:13)),
              ),
              _saving
                  ...
                  label:Text(localization.get('save'),style:const TextStyle(color:Colors.white,fontWeight:FontWeight.bold,fontSize:14))),
  ...
  Widget _buildHero(){
    return Container(
      ...
      child:SafeArea(child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
        ...
        Text(_name.isNotEmpty ? _name : localization.get('your_clinic'),
            style:const TextStyle(fontSize:26,fontWeight:FontWeight.bold,color:Colors.white)),
        const SizedBox(height:4),
        Text(localization.get('tap_logo_hint'),
            style:const TextStyle(fontSize:13,color:Colors.white70)),
        const SizedBox(height:20),
        Padding(
          padding:const EdgeInsets.symmetric(horizontal:20),
          child:Row(children:[
            Expanded(child:_qBtn(Icons.phone, localization.get('btn_call'))),         const SizedBox(width:12),
            Expanded(child:_qBtn(Icons.chat, localization.get('btn_whatsapp'))),      const SizedBox(width:12),
            Expanded(child:_qBtn(Icons.calendar_today, localization.get('btn_book'))),const SizedBox(width:12),
            Expanded(child:_qBtn(Icons.map, localization.get('btn_map'))),
          ]),
        ),
        const SizedBox(height:10),
      ])),
  ...
  Widget _tabAbout(){
    return SingleChildScrollView(
      ...
      child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        ...
        _secTitle(localization.get('about_us'),Icons.info_outline),
        ...
        _card(TextField(controller:_aboutC,maxLines:6,
            ...
            decoration:InputDecoration(border:InputBorder.none,
                hintText:localization.get('describe_clinic'),
                hintStyle:const TextStyle(color:Color(0xFFAAAAAA))))),
        const SizedBox(height:24),

        _secTitle(localization.get('technology_equipment'),Icons.computer_outlined),
        const SizedBox(height:12),
        ..._kTech.map((tech){
        ...
        Row(children:[Expanded(child:_secTitle(localization.get('certifications_awards'),Icons.workspace_premium_outlined))]),
        const SizedBox(height:12),
        GridView.builder(
        ...
        _secTitle(localization.get('insurance_payment'),Icons.account_balance_wallet),
        const SizedBox(height:12),
        _card(Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
          Text(localization.get('insurance_accepted'),style:const TextStyle(fontSize:13,color:Color(0xFF999999))),
          const SizedBox(height:10),
          _chips(_kIns,_ins),
          const Divider(height:24),
          Text(localization.get('payment_methods'),style:const TextStyle(fontSize:13,color:Color(0xFF999999))),
          const SizedBox(height:10),
          _chips(_kPay,_pay),
        ])),
        const SizedBox(height:24),

        _secTitle(localization.get('languages_spoken'),Icons.language),
        const SizedBox(height:12),
        _card(_chips(_kLang,_lang)),
        const SizedBox(height:24),

        _secTitle(localization.get('parking_accessibility'),Icons.local_parking),
        const SizedBox(height:12),
  ...
  Widget _addCertCell()=>GestureDetector(
    ...
    child:Container(decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(12),
        ...
        child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
          const Icon(Icons.add_photo_alternate,color:Color(0xFF7DD3C0),size:28),
          const SizedBox(height:6),
          Text(localization.get('add'),style:const TextStyle(fontSize:12,color:Color(0xFF7DD3C0),fontWeight:FontWeight.w600)),
        ])),
  );
  ...
  Future<void> _addCert() async {
    ...
    try{
      ...
      if(mounted){setState(()=>_saving=false);ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('${localization.get('upload_failed')}: $e'),backgroundColor:Colors.red));}
    }
  }
  ...
  void _certMenu(int i){
    showModalBottomSheet(context:context,shape:const RoundedRectangleBorder(borderRadius:BorderRadius.vertical(top:Radius.circular(20))),
      builder:(_)=>SafeArea(child:Column(mainAxisSize:MainAxisSize.min,children:[
        ListTile(leading:const Icon(Icons.visibility,color:Color(0xFF7DD3C0)),title:Text(localization.get('view')),
        ...
        ListTile(leading:const Icon(Icons.edit,color:Color(0xFF7DD3C0)),title:Text(localization.get('replace')),
            onTap:() async{
        ...
        ListTile(leading:const Icon(Icons.delete,color:Colors.red),title:Text(localization.get('remove'),style:const TextStyle(color:Colors.red)),
        ...
        return Container(
          ...
          child:Row(children:[
            ...
            PopupMenuButton<String>(
              ...
              itemBuilder:(_)=>[
                PopupMenuItem(value:'edit',child:Text(localization.get('edit'))),
                PopupMenuItem(value:'remove',child:Text(localization.get('remove'),style:const TextStyle(color:Colors.red))),
              ],
            ),
          ]),
        );
    ...
    child:GestureDetector(onTap:()=>_editMember(null),
      child:Container(
        ...
        child:Row(mainAxisAlignment:MainAxisAlignment.center,children:[
          const Icon(Icons.add,color:Color(0xFF7DD3C0),size:22),const SizedBox(width:8),
          Text(localization.get('add_team_member'),style:const TextStyle(color:Color(0xFF7DD3C0),fontWeight:FontWeight.bold,fontSize:15)),
        ]),
      ),
    ),
  );
  ...
  void _editMember(int? idx){
    ...
    showDialog(context:context,builder:(_)=>AlertDialog(
      ...
      title:Text(idx==null ? localization.get('add_team_member_title') : localization.get('edit_team_member_title'),
          style:const TextStyle(fontWeight:FontWeight.bold,fontSize:17)),
      content:SingleChildScrollView(child:Column(mainAxisSize:MainAxisSize.min,children:[
        TextField(controller:nc,decoration:_dec(localization.get('full_name'),icon:Icons.person)),
        const SizedBox(height:12),
        TextField(controller:rc,decoration:_dec(localization.get('role_hint'),icon:Icons.work_outline)),
        const SizedBox(height:12),
        TextField(controller:ec,decoration:_dec(localization.get('experience_hint'),icon:Icons.timer_outlined)),
        const SizedBox(height:12),
        TextField(controller:sc,decoration:_dec(localization.get('specializations_hint'),icon:Icons.star_outline)),
      ])),
      actions:[
        TextButton(onPressed:()=>Navigator.pop(context),child:Text(localization.get('cancel'),style:const TextStyle(color:Color(0xFF999999)))),
        ElevatedButton(
          onPressed:(){
          ...
          child:Text(localization.get('save'),style:const TextStyle(color:Colors.white,fontWeight:FontWeight.bold)),
        ),
      ],
    ));
  }
    ...
    return ListView.builder(
      ...
      itemBuilder:(ctx,i){
        if(i==_gal.length) return GestureDetector(
          onTap:()=>setState(()=>_gal.add(_GalCat(id:DateTime.now().millisecondsSinceEpoch.toString(),name:'${localization.get('tab_gallery')} ${_gal.length+1}',urls:[]))),
          child:Container(
            padding:const EdgeInsets.symmetric(vertical:14),
            decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(16),
                border:Border.all(color:const Color(0xFF7DD3C0).withOpacity(0.4),width:1.5),
            ...
            child:Row(mainAxisAlignment:MainAxisAlignment.center,children:[
              const Icon(Icons.add,color:Color(0xFF7DD3C0),size:22),const SizedBox(width:8),
              Text(localization.get('add_category'),style:const TextStyle(color:Color(0xFF7DD3C0),fontWeight:FontWeight.bold,fontSize:15)),
            ]),
          ),
        );
        ...
        child:Row(children:[
          Expanded(
            child:TextField(
              ...
              decoration:InputDecoration(border:InputBorder.none,hintText:localization.get('category_name'),
                  suffixIcon:const Icon(Icons.edit,size:14,color:Color(0xFF7DD3C0))),
            ),
          ),
          Container(padding:const EdgeInsets.symmetric(horizontal:10,vertical:4),
              decoration:BoxDecoration(color:const Color(0xFFA8E6CF).withOpacity(0.2),borderRadius:BorderRadius.circular(12)),
              child:Text('${cat.urls.length} ${localization.get('photos')}',style:const TextStyle(fontSize:12,fontWeight:FontWeight.w600,color:Color(0xFF7DD3C0)))),
        ...
        itemBuilder:(ctx,pi){
          if(pi==cat.urls.length) return GestureDetector(
            ...
            child:Container(decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(12),
                ...
                boxShadow:[BoxShadow(color:Colors.black.withOpacity(0.05),blurRadius:8,offset:const Offset(0,2))]),
                child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
                  const Icon(Icons.add_photo_alternate,color:Color(0xFF7DD3C0),size:26),
                  const SizedBox(height:4),
                  Text(localization.get('add_photo'),style:const TextStyle(fontSize:11,color:Color(0xFF7DD3C0))),
                ])),
          );
          return GestureDetector(
            onTap:()=>showModalBottomSheet(context:context,shape:const RoundedRectangleBorder(borderRadius:BorderRadius.vertical(top:Radius.circular(20))),
                builder:(_)=>SafeArea(child:Column(mainAxisSize:MainAxisSize.min,children:[
                  ListTile(leading:const Icon(Icons.delete,color:Colors.red),title:Text(localization.get('remove_photo'),style:const TextStyle(color:Colors.red)),
  ...
  Widget _tabContact(){
    return SingleChildScrollView(
      ...
      child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        ...
        _secTitle(localization.get('opening_hours'),Icons.access_time),
        const SizedBox(height:12),
        _card(Column(children:_kDays.map((d)=>_dayRow(d,d!=_kDays.last)).toList())),
        const SizedBox(height:24),

        _secTitle(localization.get('contact_info'),Icons.contact_mail),
        const SizedBox(height:12),
        _editContactRow(Icons.location_on, localization.get('address'), _addrC),
        const SizedBox(height:12),
        _editContactRow(Icons.phone, localization.get('phone'), _phoneC, type:TextInputType.phone),
        const SizedBox(height:12),
        _editContactRow(Icons.email, localization.get('email'), _emailC, type:TextInputType.emailAddress),
        const SizedBox(height:20),
      ]),
    );
  }
    ...
    return Padding(padding:const EdgeInsets.only(bottom:8),
      child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        Row(children:[
          ...
          SizedBox(width:52,child:Text(h.open ? localization.get('open') : localization.get('closed'),
              style:TextStyle(fontSize:12,color:h.open?const Color(0xFF7DD3C0):const Color(0xFFAAAAAA)))),
        ]),
        if(h.open)...[
          const SizedBox(height:4),
          Row(children:[
            ...
            GestureDetector(onTap:()=>setState(()=>h.brk=!h.brk),
                child:Row(mainAxisSize:MainAxisSize.min,children:[
                  ...
                  const SizedBox(width:4),
                  Text(localization.get('break_label'),style:const TextStyle(fontSize:11,color:Color(0xFF777777))),
                ])),
          ]),
          if(h.brk)...[
            const SizedBox(height:4),
